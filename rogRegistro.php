<?php
// rogRegistro
// Objeto para manipular Datos en PHP
//
include_once("../include/adodb/adodb.inc.php");

function alert($msj) { echo "<script>alert('$msj')</script>"; }

function confirm($msj,$Si,$No) {
    echo "<script>if(confirm('$msj')) $Si else $No</script>";
    return $respuesta;
}
...
Class rogCampo {
    ...
    function __construct($padre, $nb, $valor, $tipo = "text", $obligatorio = false, $match = "", $validador = "", $titulo = null) {
        $this->padre = $padre;
        $this->msj("Creando <strong>$nb</strong>");
//        foreach (func_get_args() as $prm) {
//            echo $prm, "<br/>";
//        }
//        $this->msj(func_num_args() ." argumentos");
        
        $this->nb = $nb;
        $this->titulo = (is_null($titulo) ? $nb : $titulo);
        $this->tipo = $tipo;
...
//        $this->msj($this);
    ...
    public function valida($valor) {
        $this->msj("Valida <strong>$this->nb</strong>: valor: $valor.");
        ...
        if (!$err) { // Función validadora
            if($this->validador) {
                $this->msj("Llama a la función validadora");
                $validador = $this->validador;
                $err = $validador($valor);
                $this->msj("Resultado: '$err'");
            }
        }
        $this->Err = $err;
        $this->contenido = $valor;
        $this->msj("La validación " .(empty($err) ? "fue exitosa." : "falló"));
        return empty($err);
    }
  
    public function valor($valor = null) {
    ...
    public function datos() {
        return array ($this->nb => $this->contenido);
    }
  
    public function toString() { return $this->titulo .": " .$this->contenido; }
    
    private function Combo ($match) {
        // @param match arreglo con los valores [etiqueta => valor] a llenar el combo
        $txt = "<select name='$this->nb'>";
        $opciones = "";
        foreach($match as $etiqueta => $valor) {
            $chq = ($this->contenido == $valor ? "selected" : "");
            $opciones .= "<option $chq value='$valor'>$etiqueta</option>";
        }
        return $txt .$opciones ."</select>";
    }

    private function Tabla ($match) {
    ...
    private function radioBtn ($match) {
        // @param match  arreglo de tipo [etiqueta => valor] para crear las opciones "radio button"
        $opciones = "";
        foreach($match as $etiqueta => $valor) {
            $chq = ($this->contenido == $valor ? "checked" : "");
            $opciones .= "<input type='radio' name='$this->nb' $chq value='$valor'/>$etiqueta";
        }
        return $opciones;
    }

    public function Input() {
        $this->msj("$this->nb: tipo: $this->tipo, titulo: $this->titulo");
        ...
        $txt .= "<br><br>";
        //    echo $this->nb, ".   ", htmlspecialchars($txt), "<br/>";
        return $txt;
    }
